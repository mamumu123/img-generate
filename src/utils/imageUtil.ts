  ...
  if (
    ...
    if (aboveA === 255 || belowA === 0) {
      ...
      const newColorA = Math.max(
        Math.min(
          Math.floor(
            (aboveDiaphaneity + belowDiaphaneity * (1 - aboveDiaphaneity)) *
              255,
          ),
          255,
        ),
  ...
  if (imageData) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        // (y * width + x) = (y * width + width - 1 - x )
        const startIndex = (y * width + x) * 4; // 每一个数据描述一个点位 ， r/g/b/a
...
// 修改尺寸
// 通过画布实现，将图片画到指定大小的 canvas ，可以实现修改尺寸的需求
